	...
	<div class="row">
		<div class="col-md-12">
			<div class="box">
				...
				<div class="box-body">
					<form action="{{ route('product_type.update', ['id' => $detail['id']]) }}" method="post" enctype="multipart/form-data">
						{{ csrf_field() }}
						<div class="row">
...
<?php 
    function build_new_category($categorie, $parent_id = 0, $char = '', $check){
        $cate_child = array();
        ...
        if ($cate_child){
            ...
            ?>
            <option value="<?php echo $value['id'] ?>" <?php echo ($check == $value['id'])? 'selected' : '' ?> ><?php echo $char.$value['title'] ?></option>
            <?php build_new_category($categorie, $value['id'], $char.'---|', $check) ?>
            <?php
            }
        }
    }
